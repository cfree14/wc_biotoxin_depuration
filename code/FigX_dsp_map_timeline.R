...
data <- data_orig %>% 
  ...
  left_join(species %>% select(comm_name, comm_name_use1, comm_name_use2), by=c("comm_name")) %>% 
  # Order
...
g2 <- ggplot() +
  ...
  geom_point(data_lo, mapping=aes(x=date, 
                       y=lat_dd,
                       color=comm_name_use2), alpha=0.5, size=0.4, pch=16) +
  # Above
...
g4 <- ggplot(max_yr, aes(x=toxicity_ug_100g_max,
...
# Export
ggsave(g, filename=file.path(plotdir, "FigX_dsp_map_timeline.png"), 
       width=8.5, height=8.25, units="in", dpi=600)
