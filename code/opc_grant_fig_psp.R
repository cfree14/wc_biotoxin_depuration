...
# Read data
da_orig <- readRDS(file=file.path(outdir, "WC_psp_data.Rds"))
...
da <- da_orig %>% 
  ...
  # Simplify
  select(date, comm_name, lat_dd, long_dd, toxicity_ug_100g) %>% 
  na.omit()
...
da_spp <- da %>% 
  group_by(comm_name) %>% 
  summarize(n=n(),
            toxicity_ug_100g=max(toxicity_ug_100g)) %>% 
  ungroup() %>% 
  arrange(desc(n)) %>% 
  # Classify
  mutate(comm_name1=ifelse(toxicity_ug_100g<80 | n<5 | comm_name=="Mediterranean/Pacific blue/blue mussels", "Other", comm_name))
...
# Export
ggsave(g, filename=file.path(plotdir, "FigX_recent_data_psp.png"), 
